...
from data_utils import create_input, iobes_iob, check_iob
...
class Model(object):
    ...
    def predict(self, sess, batch, label_vocab):
        ...
        for i in range(len(strings)):
            ...
            pred = iobes_iob([label_vocab[int(x)]
    ...
    def evaluate(self, sess, data_manager, label_vocab):
        """
        ...
        for batch in data_manager.iter_batch():
            ...
            for i in range(len(strings)):
                result = []
                string = strings[i][:lengths[i]]
                gold = iobes_iob([label_vocab.get_word(int(x)) for x in tags[i][:lengths[i]]])
                pred = iobes_iob([label_vocab.get_word(int(x)) for x in batch_paths[i][:lengths[i]]])
